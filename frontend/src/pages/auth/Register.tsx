  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm, setSubmitting }) => {
      ...
      try {
        ...
        // Store a dummy session to allow immediate dashboard access
        localStorage.setItem('accessToken', 'dummy-token');
        localStorage.setItem('userEmail', values.email);
        localStorage.setItem('userName', values.fullName);
        localStorage.setItem('userRole', values.role.toLowerCase());
        setApiSuccess('Account created successfully! Redirecting to dashboard...');
        resetForm();
        // Navigate to appropriate dashboard based on role
        const targetPath = values.role.toLowerCase() === 'candidate' ? '/candidate' : '/';
