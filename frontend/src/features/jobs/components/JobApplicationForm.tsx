...
import { ChevronLeft, UploadCloud, X, FileText, CheckCircle, Loader2 } from 'lucide-react';
...
export const JobApplicationForm = ({
  ...
  const handleSubmit = async (e: { preventDefault: () => void }) => {
    ...
    try {
      ...
      if (useNewResume && newResumeFile) formData.append('resume', newResumeFile);
      ...
      <div className="animate-fade-in" style={{ padding: '0 0.5rem', maxWidth: '800px', margin: '0 auto' }}>
        <div className="card" style={{ padding: '3rem', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem', fontSize: '1rem' }}>
            Your application for <strong style={{ color: 'var(--text-primary)' }}>{job.title}</strong> at <strong style={{ color: 'var(--text-primary)' }}>{job.company}</strong> has been submitted. Our HR team will review it shortly.
          </p>
          <button
            onClick={onCancel}
    ...
    <div className="animate-fade-in" style={{ padding: '0 0.5rem', maxWidth: '800px', margin: '0 auto' }}>
      <div className="card" style={{ padding: '2.5rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
          ...
          <button
            onClick={onCancel}
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border)', color: 'var(--text-secondary)', cursor: 'pointer', padding: '0.5rem', borderRadius: '50%' }}
          >
            <X size={16} />
          </button>
        </div>
        ...
        {error && (
          <div style={{ marginBottom: '1.5rem', padding: '0.75rem 1rem', borderRadius: '0.5rem', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', color: '#ef4444', fontSize: '0.875rem' }}>
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <label style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', fontWeight: 500 }}>LinkedIn (Optional)</label>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <label style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', fontWeight: 500 }}>Portfolio (Optional)</label>
              <input type="url" placeholder="https://yourportfolio.com" value={form.portfolio}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            ...
            <div
              ...
              onMouseEnter={(e) => { if (!useNewResume) e.currentTarget.style.borderColor = 'var(--accent)'; }}
              onMouseLeave={(e) => { if (!useNewResume) e.currentTarget.style.borderColor = 'var(--border)'; }}
            >
